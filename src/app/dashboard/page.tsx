...
const StatCard = ({ label, value, icon: Icon, variant, delay }: any) => {
  const styles: any = {
    blue: { iconBg: "bg-blue-50", iconColor: "text-blue-600", hoverBorder: "hover:border-blue-200", shadow: "shadow-blue-100" },
    green: { iconBg: "bg-green-50", iconColor: "text-green-600", hoverBorder: "hover:border-green-200", shadow: "shadow-green-100" },
    yellow: { iconBg: "bg-yellow-50", iconColor: "text-yellow-600", hoverBorder: "hover:border-yellow-200", shadow: "shadow-yellow-100" },
    orange: { iconBg: "bg-orange-50", iconColor: "text-orange-600", hoverBorder: "hover:border-orange-200", shadow: "shadow-orange-100" },
  };
  const theme = styles[variant] || styles.blue;
  return (
    <div
      className={cn("group bg-white p-5 rounded-2xl border border-gray-100 shadow-sm transition-all duration-300 ease-out hover:-translate-y-1 hover:shadow-lg", theme.hoverBorder, "flex flex-col justify-between h-32 cursor-default relative overflow-hidden")}
      style={{ animationDelay: `${delay}ms` }}
    >
      <div className="flex justify-between items-start relative z-10">
        <div className={cn("p-2.5 rounded-xl transition-transform duration-300 group-hover:scale-110", theme.iconBg, theme.iconColor)}>
          <Icon size={20} strokeWidth={2.5} />
        </div>
        <div className={cn("opacity-0 group-hover:opacity-100 transition-opacity duration-300", theme.iconColor)}>
          <TrendingUp size={16} />
        </div>
      </div>
      <div className="relative z-10">
        <h3 className="text-3xl font-bold text-[#2F4157] tracking-tight group-hover:translate-x-1 transition-transform duration-300">{value}</h3>
        <p className="text-xs font-bold text-gray-400 uppercase tracking-wider mt-1 group-hover:text-gray-500 transition-colors">{label}</p>
      </div>
      <div className={cn("absolute -right-4 -bottom-4 w-20 h-20 rounded-full opacity-0 group-hover:opacity-20 transition-all duration-500 blur-xl", theme.iconBg)} />
    </div>
  );
};
...
const ActionButton = ({ icon: Icon, label, subLabel, href, isLocked, onClick, tier }: any) => {
  const content = (
    <div className={cn("relative overflow-hidden group w-full p-4 rounded-2xl border transition-all duration-200 text-left", isLocked ? "bg-gray-50 border-gray-200 opacity-80" : "bg-white border-gray-100 hover:border-[#E56668]/30 hover:shadow-md active:scale-[0.98]")}>
      <div className="flex items-center gap-4 relative z-10">
        <div className={cn("w-12 h-12 rounded-xl flex items-center justify-center text-xl transition-colors", isLocked ? "bg-gray-200 text-gray-400" : "bg-[#F5F7FA] text-[#2F4157] group-hover:bg-[#E56668] group-hover:text-white")}>
          <Icon size={20} />
        </div>
        <div className="flex-1">
          <h4 className={cn("font-bold text-sm flex items-center gap-2", isLocked ? "text-gray-500" : "text-[#2F4157]")}>
            ...
            {isLocked && tier === "insider" && (
              <span className="text-[9px] font-black bg-yellow-100 text-yellow-700 px-2 py-0.5 rounded-full border border-yellow-300">VISIONARY ONLY</span>
            )}
          </h4>
          <p className="text-[11px] text-gray-400 leading-tight mt-0.5">{subLabel}</p>
        </div>
        {isLocked ? <Lock size={16} className="text-gray-300" /> : <ChevronRight size={16} className="text-gray-300 group-hover:text-[#E56668] group-hover:translate-x-1 transition-transform" />}
      </div>
    </div>
  );
...
function DashboardContent() {
  ...
  // ✅ PERF FIX: Semua query jalan PARALEL via Promise.all
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);

      // Auth tetap harus duluan (wajib punya user.id untuk query lainnya)
      ...
      // ✅ PERF FIX #1: Tembak 3 query SEKALIGUS, bukan satu-satu
      // Estimasi waktu: ~300-500ms (1 round trip) vs ~900-1500ms (3 round trips sequential)
      const [membershipRes, userRes, registrationsRes] = await Promise.all([
        supabase
          .from("memberships")
      ...
      // ✅ PERF FIX #2: Update semua state dalam satu setData call — menghindari multiple re-renders
      setData({
        user: {
          id: authUser.id,
          name: userRes.data?.full_name || authUser.user_metadata?.full_name || "Learner",
          tier: uiTier,
          avatar: authUser.user_metadata?.avatar_url || "",
          institution: userRes.data?.user_profiles?.[0]?.institution || "IELS Community",
          batch: userRes.data?.user_profiles?.[0]?.batch || "2026 Cohort",
        },
        stats: {
          events_attended: eventsAttended,
          contributions: 0,
          hours: eventsAttended * 2,
          streak: 1,
        },
        activeGoal: null,
        upcomingEvents: [],
  ...
  }, []); // ✅ PERF FIX #3: Kosongkan deps — fetchData cukup run sekali saat mount
...
<div className="relative rounded-[32px] overflow-hidden bg-[#304156] text-white shadow-lg border border-white/10">
  {/* Decorative blurs */}
  <div className="absolute top-0 right-0 w-[400px] h-[400px] bg-[#577E90]/10 rounded-full blur-3xl -translate-y-1/2 translate-x-1/3 pointer-events-none" />
    ...
    <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-3">
          ...
          <span className="text-white/60 text-xs font-semibold tracking-wider uppercase">{data.user.batch}</span>
        </div>
        ...
        <p className="text-white/80 max-w-lg text-base lg:text-lg leading-relaxed">
          Your learning journey continues. Track your progress, achieve your goals, and unlock new opportunities.
        </p>
      </div>
      ...
      <div className="flex items-center gap-3">
        {isExplorer && (
          <button
            ...
            className="flex items-center justify-center gap-2 px-8 py-4 rounded-2xl bg-[#577E90] text-white font-semibold shadow-lg hover:bg-[#486b7a] hover:scale-[1.02] hover:-translate-y-0.5 active:scale-[0.96] transition-all duration-200 group"
          >
        ...
        {isInsider && (
          <button
            onClick={() => setShowProModal(true)}
            className="flex items-center justify-center gap-2 px-8 py-4 rounded-2xl bg-[#577E90] text-white font-semibold shadow-lg hover:bg-[#486b7a] hover:scale-[1.02] transition-all duration-200 group"
          >
            <Crown size={18} className="group-hover:rotate-12 transition-transform duration-300" fill="currentColor" />
            Unlock Visionary
          </button>
        )}
      </div>
          ...
          <div className="grid lg:grid-cols-3 gap-8 items-start">
                      ...
                      <div key={event.id} className="group relative bg-white border border-gray-100 rounded-2xl p-5 shadow-sm hover:shadow-md hover:border-[#E56668]/20 transition-all duration-300 flex items-start gap-6">
                        <div className="flex-shrink-0 w-20 h-20 bg-white border border-gray-200 rounded-2xl overflow-hidden flex flex-col shadow-sm group-hover:border-[#E56668]/30 transition-colors">
                          ...
                          <div className="flex-1 flex items-center justify-center bg-white group-hover:bg-[#FFFDFD] transition-colors">
                            <span className="text-3xl font-bold text-[#2F4157] tracking-tight">{new Date(event.date).getDate()}</span>
                          </div>
                        </div>

                        <div className="flex-1 min-w-0 py-1">
                          <div className="flex flex-wrap items-center gap-2 mb-2">
                            <span className="px-2 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-wide border bg-gray-50 text-gray-600 border-gray-200">{event.type}</span>
                            {event.isRegistered ? (
                              ...
                              <span className="flex items-center gap-1 text-[10px] font-bold text-gray-400 bg-gray-50 px-2 py-1.5 rounded-full border border-gray-100">Open</span>
                            )}
                          </div>
                          <h3 className="font-bold text-[#2F4157] text-base md:text-lg leading-snug mb-1.5 group-hover:text-[#E56668] transition-colors line-clamp-1 pr-8">{event.title}</h3>
            ...
            <div className="space-y-6">
              ...
              <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
                ...
                <div className="space-y-3">
                  <ActionButton icon={Target} label="My Goals" subLabel="Track your learning journey" href="/dashboard/goals" tier={data.user.tier} />
                  <ActionButton icon={Star} label="My Portfolio" subLabel="Manage your verifiable CV" href="/dashboard/portfolio" isLocked={isExplorer} onClick={() => setShowProModal(true)} tier={data.user.tier} />
                  <ActionButton icon={Calendar} label="Book Mentorship" subLabel={isVisionary ? "1-on-1 with principals unlocked!" : "1-on-1 with experts"} href="/dashboard/mentorship" isLocked={!isVisionary} onClick={() => setShowProModal(true)} tier={data.user.tier} />
                  <ActionButton icon={MoreHorizontal} label="Resources Hub" subLabel="Access guidelines & tools" href="/dashboard/resources" tier={data.user.tier} />
                </div>
                ...
                <div className="relative overflow-hidden rounded-2xl bg-[#2F4157] text-white p-6 shadow-xl group cursor-pointer" onClick={() => setShowProModal(true)}>
                  ...
                  <div className="relative z-10">
                    ...
                    <p className="text-sm text-white/70 mb-4 leading-relaxed">
                      {isExplorer ? "Insider members get access to locked opportunities and priority mentorship." : "Visionary members get exclusive 1-on-1 sessions with IELS Principals."}
                    </p>
          ...
          <div className="relative overflow-hidden rounded-3xl bg-gradient-to-r from-[#2F4055] via-[#914D4D] to-[#304156] text-white shadow-2xl group ring-1 ring-white/10 font-geologica">
            ...
            <div className="relative z-10 p-8 md:p-10 flex flex-col md:flex-row items-center justify-between gap-8">
              ...
              <div className="shrink-0 w-full md:w-auto">
                ...
                <p className="text-white/60 text-[10px] text-center mt-3 font-medium tracking-wide">*Limited slots available for Fast Track</p>
              </div>
            </div>
          </div>
        </div>
