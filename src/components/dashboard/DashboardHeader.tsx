...
export default function DashboardHeader({ 
  ...
  return (
    <header className="sticky top-0 z-40 w-full bg-[#2F4157] shadow-lg border-b border-white/10 transition-all duration-300">
        ...
        <div className="flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-1 justify-center flex-1 px-4">
            {navItems.map((item) => {
              ...
              const isActive = isDashboardRoot || isStandardActive || isChildActive;
      ...
      {/* ── MOBILE MENU (slide-in dari kanan, sama persis style public Header) ── */}
      {isMobileOpen && (
      ...
      <div className={`md:hidden fixed top-0 right-0 h-full w-[80%] max-w-sm bg-[#2F4157] z-50 transition-transform duration-300 shadow-2xl ${isMobileOpen ? "translate-x-0" : "translate-x-full"}`}>
        ...
        <nav className="p-6 space-y-2 overflow-y-auto h-[calc(100vh-80px)]">
              ...
              return (
                <div key={item.name} className="overflow-hidden">
                  <div className={`rounded-xl transition-colors ${isActive ? "bg-white/10" : ""}`}>
                    ...
                    <div className={cn(
                      "overflow-hidden transition-all duration-300 ease-in-out pl-4 pr-2 space-y-1",
            ...
            return (
              <Link
                ...
                className={cn(
                  ...
                  isActive ? "bg-white text-[#2F4157]" : "text-white hover:bg-white/10"
                )}
              >
                <Icon size={20} className={isActive ? "text-[#E56668]" : "text-gray-300"} />
                {item.name}
          ...
          {/* Profile section — collapsible, sama style kayak Learning dropdown */}
          <div className="pt-4 mt-4 border-t border-white/10">
            <div className="overflow-hidden">
              <div className={`rounded-xl transition-colors ${openDropdown === "profile" ? "bg-white/10" : ""}`}>
                
                {/* User card — jadi trigger dropdown */}
                <button
                  onClick={() => setOpenDropdown(openDropdown === "profile" ? null : "profile")}
                  className="w-full flex items-center justify-between pr-2 rounded-xl hover:bg-white/5 transition-colors"
                >
                  <div className="flex items-center gap-3 px-4 py-3">
                    ...
                    <div className="text-left min-w-0">
                      {/* Ambil dua kata pertama dari nama */}
                  ...
                  <ChevronDown
                    size={18}
                    className={cn("text-gray-400 transition-transform duration-300 mr-1", openDropdown === "profile" && "rotate-180")}
                  />
                </button>
                ...
                <div className={cn(
                  "overflow-hidden transition-all duration-300 ease-in-out pl-4 pr-2 space-y-1",
                  openDropdown === "profile" ? "max-h-96 pb-3 opacity-100" : "max-h-0 opacity-0"
                )}>
                  <Link href="/dashboard/profile" onClick={closeMobileMenu} className="flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm text-white/80 hover:bg-white/10 hover:text-white transition-colors">
                    <User size={16} />
                    My Profile
                  </Link>
                  <Link href="/dashboard/settings" onClick={closeMobileMenu} className="flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm text-white/80 hover:bg-white/10 hover:text-white transition-colors">
                    <Settings size={16} />
                    Settings
                  </Link>
                  <Link href="/dashboard/help" onClick={closeMobileMenu} className="flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm text-white/80 hover:bg-white/10 hover:text-white transition-colors">
                    <HelpCircle size={16} />
                    Help Center
                  </Link>
                </div>

                {/* Sign Out — di luar dropdown, selalu visible */}
                <button
